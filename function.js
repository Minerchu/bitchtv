...
function viewComment(id,page){
	...
	ajax.get( 
		...
		function(obj) {
			if (obj.responseText=="err"){
				set(document.getElementById("comment_list"),"<font color='red'>发生错误惹</font>")	
			}else{
...
function submitComment(id){
	if(document.getElementById("username").value.length<1){alert('请填写昵称惹');return false;}
	if(document.getElementById("msg").value.length<1){alert('请填写内容惹');return false;}
	ajax.postf(
		document.getElementById("f_comment"),
		function(obj){if(obj.responseText=="ok"){viewComment(id,1);alert('姐姐感谢您的评论惹!');}else if(obj.responseText=="validateerr"){alert('验证码错误惹，请点击验证码图片更新验证码！');}else if(obj.responseText=="havecomment"){alert('姐姐你手也太快了，歇会儿再来评论惹！');}else if(obj.responseText=="ipcomment"){alert('您所在的ip不能评论');}else if(obj.responseText=="wordcomment"){alert('您的评论中有禁用词语，不能评论惹');}/*else{alert(obj.responseText);}*/}
	);
}
...
function diggVideo(id,div){
	ajax.get(
		...
		function (obj){
			...
			if (!isNaN(returnValue)){set(document.getElementById(div),returnValue);alert('(*^__^*) 嘻嘻……，顶得我真舒服惹！');}else if(obj.responseText=="err"){alert('顶失败')}else if(obj.responseText=="havescore"){alert('(*^__^*) 嘻嘻…… 这么热心啊，您已经顶过惹！')}	
		}
	);	
}
...
function treadVideo(id,div){
	ajax.get(
		...
		function (obj){
			var returnValue=Number(obj.responseText)
			if(!isNaN(returnValue)){set(document.getElementById(div),returnValue);alert('小样儿，居然敢踩我惹！');}else if(obj.responseText=="err"){alert('踩失败惹')}	else if(obj.responseText=="havescore"){alert('我晕，您已经踩过了，想踩死我啊！')}	
		}
	);	
}
...
function diggNews(id,div){
	ajax.get("/"+sitePath+"include/ajax.php?id="+id+"&action=diggnews",function (obj){
			var returnValue=Number(obj.responseText)
			if (!isNaN(returnValue)){set(document.getElementById(div),returnValue);alert('(*^__^*) 嘻嘻……，顶得我真舒服惹！');}else if(obj.responseText=="err"){alert('顶失败')}else if(obj.responseText=="havescore"){alert('(*^__^*) 嘻嘻…… 这么热心啊，您已经顶过了惹！')}	
		}
	);
}
	...
	this.style.behavior='url(#default#homepage)';this.setHomePage(url);}
            ...
            if(window.netscape){
                try{netscape.security.PrivilegeManager.enablePrivilege("UniversalXPConnect");}  
                   catch (e){alert("此操作被浏览器拒绝惹！请手动设置");}
